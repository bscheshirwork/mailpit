	...
	<template v-else>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						Это отметит {{ formatNumber(mailbox.unread) }} <span v-if="mailbox.unread % 10 === 1"
							>письмо</span
						><span v-else-if="mailbox.unread % 10 === 2 || mailbox.unread % 10 === 3 || mailbox.unread % 10 === 4"
							>письма</span
						><span v-else
							>писем</span
						>
						прочитанным<span v-if="mailbox.unread > 1"
							>и</span
						>.
					</div>
		...
		>
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 id="DeleteAllModalLabel" class="modal-title">Удалить все сообщения?</h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div class="modal-body">
						Это навсегда удалит {{ formatNumber(mailbox.total) }} <span
							v-if="mailbox.total % 10 === 1"
							>сообщение</span><span
							v-else-if="mailbox.total % 10 === 2 || mailbox.total % 10 === 3 ||mailbox.total % 10 === 4"
							>сообщения</span><span
							v-else="mailbox.total > 1"
							>сообщений</span
						>.
